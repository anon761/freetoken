...
# --- 3b. NCCL link sanity --------------------------------------------------
# pynccl (the TP backend) is JIT-linked on first use and links libnccl, which the
# CUDA toolkit does not ship -- the nvidia-nccl-cuNN dependency above provides it.
# Surface an unusable link early instead of at first `ft serve --tp-size >1`.
NCCL_FLAGS="$("$VENV/bin/python" -c 'from freetoken.kernel.utils import nccl_link_flags; print(" ".join(nccl_link_flags()))' 2>/dev/null || true)"
case "$NCCL_FLAGS" in
  ""|"-lnccl")
    warn "no libnccl found (torch bundle, nvidia-nccl-cu13 or system). Tensor parallelism"
    warn "with pynccl will fail to link. Fix: '$VENV/bin/pip install nvidia-nccl-cu13'"
    warn "or set NCCL_HOME to a directory containing libnccl.so[.2]."
    ;;
  *) say "NCCL link flags: $NCCL_FLAGS" ;;
esac
