...
def _hf_config_text_only():
    """A language-model-only export: the text tower at the top level (no wrapper), carrying
    the text_config's own architectures entry, ``Qwen4ExpForCausalLM``."""
    hf = _hf_config()
    return SimpleNamespace(
        model_type="qwen4_exp_text",
        architectures=["Qwen4ExpForCausalLM"],
        quantization_config=hf.quantization_config,
        **vars(hf.text_config),
    )


def test_text_only_architecture_is_registered_without_wrapper_root():
    from freetoken.models.register import get_model_spec

    spec = get_model_spec("Qwen4ExpForCausalLM")
    assert spec.module == "freetoken.models.qwen4_exp"
    # No wrapper: the weights sit under the bare ``model.`` root.
    assert spec.checkpoint_roots == ()


def test_text_only_config_parses():
    cfg = parse_config(_hf_config_text_only())
