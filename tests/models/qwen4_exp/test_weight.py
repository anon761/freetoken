...
def checkpoint(tmp_path_factory) -> tuple[str, dict[str, torch.Tensor]]:
    ...
    # iter_weights liest die HF-Config (TP-Geometrie + Fusion-Plan) — der toy
    # Checkpoint braucht eine echte config.json (AutoConfig: model_type-Pflicht).
    from tests.models.qwen4_exp.common import hf_config

    _cfg = hf_config(num_layers=2, head_dim=HD, num_q=QH, num_kv=KVH, hidden=H)
    _cfg.linear_num_heads = KH
    _cfg.linear_head_dim = HD
    _cfg.linear_conv_kernel_dim = 4
    _cfg.linear_lower_bound = 1e-5
    _cfg.index_kpool = 2
    _cfg.index_kpool_compress = True
    _cfg.index_kpool_always_select_tail = True
    _cfg.indexer_types = ("full",)
    _cfg.indexer_rope_interleave = False
    def _jsonable(v):
    ...
    json.dump(_jsonable(vars(_cfg)), open(str(folder / "config.json"), "w"))
    raw = _raw_checkpoint()
