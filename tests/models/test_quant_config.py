...
def test_compressed_tensors_naive_quantized_is_unquantized():
    """A "naive-quantized" group (fp8 weight + bf16 per-channel .weight_scale) is
    dequantized to bf16 by the weight reader, so the model must build it UNQUANTIZED --
    mapping it to FP8_CHANNEL would leave the model expecting a weight_scale the loader
    never yields."""
    q = {
        "quant_method": "compressed-tensors",
        "config_groups": {
            "group_0": {
                ...
                "targets": ["model.language_model.layers.0.mlp.shared_expert.gate_proj"],
                "weights": {"num_bits": 8, "type": "float", "strategy": "channel"},
            },
            "group_1": {
    ...
    cfg = CompressedTensorsConfig(q)
    assert cfg.scheme_for_name("model.language_model.layers.0.mlp.shared_expert.gate_proj") is None
    assert cfg.scheme_for_name("model.language_model.layers.0.mlp.experts.0.gate_proj") is not None
    assert cfg.expert_kind() == "nvfp4"
    assert not cfg.has_kind("fp8_tensor")
