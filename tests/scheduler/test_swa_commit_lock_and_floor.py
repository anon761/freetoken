...
def _managers(window: int, num_swa_tokens: int, ps: int = 1, num_pages: int = 4096, width=2048,
    ...
    cm = CacheManager(num_pages=num_pages, page_size=ps, page_table=pt, type="swa_radix",
                      ...
                      swa_eviction_interval=eviction_interval)
...
def test_commit_locks_one_window_not_the_whole_extend(ps):
...
def test_short_final_chunk_locks_no_more_than_a_window():
    ...
    for c_last in (1, 14, 15, 16, 17, 60):
        cm, tm, _pm = _managers(window, num_swa_tokens=4096, eviction_interval=1)
...
def test_decode_survives_any_prompt_length(interval):
    ...
    for prompt in (103, 129, 200, 400):
        cm, tm, pm = _managers(window, num_swa_tokens=pool + 1, eviction_interval=interval)
...
def test_a_full_batch_decodes_on_a_pool_sized_to_the_declared_floor(n_req):
    ...
    cm, tm, pm = _managers(window, num_swa_tokens=floor + 1, eviction_interval=1)
...
def test_a_per_request_sized_pool_cannot_hold_a_full_batch():
    ...
    cm, _tm, pm = _managers(window, num_swa_tokens=per_req + 1, eviction_interval=1)
...
def test_floor_terms_and_page_rounding():
    for window in (8, 128, 1024):
        for ps in (1, 8, 64, 128):
            ...
            tail = window + 2 * ps + cache_mod._resolve_swa_eviction_interval(None)  # uncollected decode tail
