...
def test_gpu_stats_segment_appended():
    rep, logs, clock = _reporter(interval=1)
    rep.gpu_stats = lambda: "TP=1 | GPU0 VRAM 1.0/2.0G 50% 40C 100W"
    rep.report_batch(_decode_batch(1), running_reqs=1, queue_reqs=0,
                     kv_used_pages=1, kv_total_pages=10, page_size=1)
    assert "gpus: TP=1 | GPU0 VRAM 1.0/2.0G 50% 40C 100W" in logs[-1]


def test_gpu_stats_absent_or_empty_adds_nothing():
    rep, logs, clock = _reporter(interval=1)
    rep.report_batch(_decode_batch(1), running_reqs=1, queue_reqs=0,
                     kv_used_pages=1, kv_total_pages=10, page_size=1)
    assert "gpus:" not in logs[-1]
    rep.gpu_stats = lambda: ""
    clock["t"] = 1.0
    rep.report_batch(_decode_batch(1), running_reqs=1, queue_reqs=0,
                     kv_used_pages=1, kv_total_pages=10, page_size=1)
    assert "gpus:" not in logs[-1]
...
def test_gpu_stats_failure_never_breaks_the_line():
    rep, logs, clock = _reporter(interval=1)
