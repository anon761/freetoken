...
def test_mtp_overlap_defers_finish_for_inflight_round():
    """MTP Phase 4 (round-as-batch): a request that terminates in the interleaved drain
    while its verify is still in flight is NOT freed there -- it is returned so
    finish_round can roll the verify tail back and free it exactly once."""
    pool, cm, tm, dm, _pm, sent, stub = _setup()
    stub.eos_token_ids = {42}  # the drained token (42) finishes the request by EOS
    req = _launch_req(pool, cm, tm, torch.arange(1, 13, dtype=torch.int32),
                      track_seqlen=8)
    ...
    deferred = Scheduler._process_last_data(
        stub, _as_last_data(Batch(reqs=[req], phase="decode")), defer_finish_reqs={req}
    )

    assert deferred == {req}
    assert req not in dm.running_reqs
    assert req.table_idx != -1 and req.mamba_ping_pong is not None  # free deferred
    assert req not in stub.finished_reqs
    assert [m.finished for m in sent] == [True]                     # terminal still shipped
    # Its pages/GDN slots stay allocated until finish_round frees them, so the pool is
    # intentionally NOT conserved here (no check_integrity).
