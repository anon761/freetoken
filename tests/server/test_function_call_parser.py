...
def test_dsv41_spaced_dsml_tags_streaming():
    # DeepSeek-V4.1 spells the DSML tags with a leading space after the marker
    # ("<｜DSML｜ calls>", "<｜DSML｜ invoke name=...>") — the same detector must
    # parse them like the compact V3.2 forms.
    parser = FunctionCallParser(OPENCODE_TOOLS, tool_call_parser="deepseekv32")
    block = (
        "<｜DSML｜ calls>\n"
        '<｜DSML｜ invoke name="glob">\n'
...
def test_dsv41_spaced_dsml_tags_detect_and_parse():
    parser = FunctionCallParser(OPENCODE_TOOLS, tool_call_parser="deepseekv32")
    text = (
        "Vorab-Text "
        "<｜DSML｜ calls>"
    ...
    result = parser.parse_non_stream(text)
    assert result.normal_text == "Vorab-Text"
    assert len(result.calls) == 1
    assert result.calls[0].name == "get_weather"
    assert json.loads(result.calls[0].parameters) == {"location": "Berlin", "days": 3}
