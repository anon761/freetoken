...
def test_small_chunks_fetch_routed_only_instead_of_whole_layers():
    """A DSpark verify (k+1 tokens) or the draft block must not pay a whole-layer
    prefill copy per layer: below tokens*top_k < num_experts the on-demand (routed)
    fetch moves strictly fewer rows, so the offload layer takes the decode path."""
    _init_tp()
    layer = _bf16_offload_layer(0, 64, 6, 8, 16)  # E=64, top_k=6

    def routed_only(tokens: int) -> bool:
        return layer._fetch_routed_only(torch.empty(tokens, 8))

    assert routed_only(1)      # decode
    assert routed_only(6)      # DSpark verify
    assert routed_only(10)     # 60 < 64
    assert not routed_only(11)  # 66 >= 64 -> whole-bank prefill
    assert not routed_only(2508)  # long prompt -> whole-bank prefill
