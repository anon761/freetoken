...
def test_swa_iface_alloc_reclaims_the_stale_window_of_a_reused_full_page():
    """Reusing a full page without freeing its window must not leak the window page: the
    allocator returns the stale page before binding a fresh one (regression for the silent
    swa-slot leak that check_integrity only catches at idle)."""
    pool, sizes = _paged_pool()
    cap = sizes.n_win_slots - P
    pool.alloc_swa(_expand([0]))
    assert pool.swa_available_size() == cap - P
    # Reuse full page 0 WITHOUT freeing its window (the leak scenario).
    pool.alloc_swa(_expand([0]))
    assert pool.swa_available_size() == cap - P, "stale window page leaked on reuse"
    assert (pool.translate_loc_from_full_to_swa(torch.arange(P)) >= 0).all()
