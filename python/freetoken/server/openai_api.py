...
def register_openai_routes(
    ...
    async def v1_models():
        ...
        return ModelList(data=[ModelCard(
            ...
            input_modalities=_input_modalities(state),
        )])
...
def _input_modalities(state: Any) -> list[str]:
    """``["text"]``, plus ``"image"`` when the vision tower is loaded for this model, so
    clients know they may send image content parts."""
    try:
        ...
        hf = cached_load_hf_config(state.config.model_path)
        if getattr(hf, "vision_config", None) is not None:
            return ["text", "image"]
    except Exception:  # noqa: BLE001 — a metadata route must never 500
        pass
    return ["text"]
