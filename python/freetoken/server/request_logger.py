...
# Resolved lazily (with ~ and $VAR expansion) so a --api-log-dir set during arg
# parsing — after this module is imported — still takes effect. Serving never
# reconfigures it at runtime otherwise.
def _resolve_log_dir() -> str | None:
    raw = os.getenv("FREETOKEN_API_LOG_DIR")
...
def _ensure_worker() -> None:
    ...
    with _init_lock:
        ...
        try:
            ...
            _log_path = os.path.join(log_dir, f"requests-{stamp}-{os.getpid()}.jsonl")
            ...
            _init_failed = True
            _warn_once(f"cannot open request log under {log_dir!r}: {exc}")
            return
