...
def launch_server(
    ...
    if server_args.gpu_memory_ratio:
        # one ratio per TP rank; resolved against the --gpu list here so a typo fails before any worker spawns
        from freetoken.gpu_select import resolve_memory_ratios

        try:
            ratios = resolve_memory_ratios(
                server_args.gpu_memory_ratio,
                server_args.gpu,
                server_args.gpu_assigned,
                server_args.tp_info.size,
                server_args.memory_ratio,
            )
        except ValueError as exc:
            raise SystemExit(f"{prog or 'ft serve'}: error: {exc}") from exc
        server_args = replace(server_args, gpu_memory_ratios=ratios)
        logger.info(
            "--gpu-memory-ratio -> "
            + ", ".join(f"TP{r}={ratio:g}" for r, ratio in enumerate(ratios))
        )
    ...
    def start_subprocess() -> "BackendHandle":
        ...
        for i in range(world_size):
            ...
            if server_args.gpu_memory_ratios is not None:
                new_args = replace(new_args, memory_ratio=server_args.gpu_memory_ratios[i])
            p = mp.Process(
