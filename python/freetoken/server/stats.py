...
class StatsTracker:
    def __init__(self, window_s: float = 5.0) -> None:
        ...
        self.vram_bytes = 0
        # MTP speculative decoding: cumulative counters (last-known-wins from the reply
        # stamps; enabled only when the head is loaded — the scheduler stamps every
        # MTP-published token, 0/0 until the first round completed).
    ...
    def observe(self, reply: Any, now: float | None = None) -> None:
        ...
        if getattr(reply, "mtp_enabled", False):
            self.mtp_enabled = True
            self.mtp_drafted = max(self.mtp_drafted, int(getattr(reply, "mtp_drafted", 0) or 0))
            self.mtp_accepted = max(self.mtp_accepted, int(getattr(reply, "mtp_accepted", 0) or 0))
...
def build_stats(state: Any, p95_ms: int, ttft_mean_ms: int) -> dict:
    ...
    return {
        ...
        "mtp": (
            {
                "drafted": tr.mtp_drafted,
                "accepted": tr.mtp_accepted,
                "accept_rate": round(tr.mtp_accepted / tr.mtp_drafted, 4)
                if tr.mtp_drafted > 0 else 0.0,
            }
            if tr.mtp_enabled else None
        ),
        "requests": {
            "active": tr.active,
