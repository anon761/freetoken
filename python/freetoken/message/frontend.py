...
class UserReply(BaseFrontendMsg):
    ...
    gpu_mem_bytes: int = 0
    # MTP speculative decoding: cumulative drafted/accepted counters (last-known-wins in
    # the stats tracker) + the head-loaded flag, so /v1/stats shows the block before the
    # first round published anything.
