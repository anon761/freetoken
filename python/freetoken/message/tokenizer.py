...
class DetokenizeMsg(BaseTokenizerMsg):
    ...
    gpu_mem_bytes: int = 0
    # MTP speculative decoding: cumulative drafted/accepted draft-token counters since
    # engine start (last-known-wins on the stats tracker). accept_rate = accepted/drafted.
    # mtp_enabled tells stats clients the head is loaded even before the first round ran.
