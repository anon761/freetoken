...
# The CacheManager takes an explicit interval (--swa-eviction-interval); None falls back to the
# FREETOKEN_SWA_EVICTION_INTERVAL env (legacy), 128 when neither is set. <= 0 disables the
# proactive out-of-window eviction entirely.
def _resolve_swa_eviction_interval(override: int | None) -> int:
...
class CacheManager:
    def __init__(self, num_pages: int, page_size: int, page_table: torch.Tensor, type: str,
        ...
        # Proactive out-of-window eviction cadence in forwards (<= 0 disables it).
        self.swa_eviction_interval = _resolve_swa_eviction_interval(swa_eviction_interval)
    ...
    def maybe_free_swa_out_of_window(self, reqs: List[Req], *, forward_iter: int) -> None:
        ...
        forward."""
        if not self.swa_paged or self.swa_eviction_interval <= 0 or forward_iter % self.swa_eviction_interval != 0:
            return
    ...
    def free_swa_out_of_window_extend(self, reqs: List[Req]) -> None:
        ...
        if not self.swa_paged or self.swa_eviction_interval <= 0:
            return
        window = self.sliding_window_size
        for req in reqs:
            floor = req.cache_handle.cached_len   # reused prefix -> its swa is tree-owned, not ours
    ...
    def rollback_last(self, req: Req, keep_to: int) -> None:
        """Free the KV pages backing the rejected positions ``[keep_to, device_len)`` of a
        speculative verify (MTP). Whole-page semantics mirror ``_padded_tail``/``allocate_paged``:
        only pages FULLY inside the rejected region return to the free list (a partial page
        holding accepted tokens keeps its slots; its rejected entries are simply never read at
        ``>= device_len`` and the page returns whole at commit/finish). ``keep_to`` must stay at
        or above the tree-owned prefix (``req.cache_handle.cached_len``). The caller shrinks
        ``req.device_len``/``input_ids`` afterwards; swa slots are refunded per token (idempotent
        over the sentinel)."""
        assert keep_to >= req.cache_handle.cached_len, (
            "rollback would free tree-owned prefix pages"
        )
        start = div_ceil(keep_to, self.page_size) * self.page_size
