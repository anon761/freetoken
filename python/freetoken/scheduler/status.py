...
class SchedulerStatusReporter:
    ...
    def _gpu_msg(self) -> str:
        ...
        Best-effort: a telemetry failure must never break the status line."""
        if self.gpu_stats is None:
            return ""
        try:
            segment = self.gpu_stats()
        except Exception:  # noqa: BLE001 -- telemetry is a nicety
            return ""
        return f", gpus: {segment}" if segment else ""
