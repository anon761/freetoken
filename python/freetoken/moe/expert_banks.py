...
class ExpertBanks:
    ...
    # True when the FTW loader already read the rank's TP band (the post-load
    # _tp_slice_ftw_sources pass must not re-slice the sliced banks)
    ...
    # The backing HostBank objects (same key/layer layout as ``sources``), so a TP>1
    # re-slice can dispose the abandoned full banks' pinned pages. None for loaders
    # that build their banks at rank width already (no re-slice happens).
    host_banks: dict[str, list] | None = field(default=None)
...
def _tp_slice_ftw_sources(sources: dict[str, list[torch.Tensor]], model_config, host_banks: dict[str, list] | None = None) -> None:
    """TP-slice the FTW banks in place (rank's intermediate band, mirrors the
    streaming loader's placement — see nvfp4_banks._tp_expert_geometry). FTW
    banks are stored TP-agnostic (full), while the MoE layer computes its local
    intermediate slice; without this the slot-cache geometry and the kernel
    shapes disagree at TP>1.

    ``host_banks`` (FTW path): the backing HostBank objects per source name. After a
    name is sliced, its FULL banks are abandoned — dispose them, or their pinned
    pages stay resident and page-locked for the process lifetime (~1.5x the serving
    set at TP=2). Names without a rank-band slice (alphas) keep their banks."""
    ...
    def _pin(t: torch.Tensor) -> torch.Tensor:
        # Materialize in cudaMallocHost storage: a plain .contiguous() copy would
        # be unpinned regular memory, and the slot-cache gather reads the banks
        # zero-copy from the GPU (pinned + device-mapped is the contract).
        # CPU rigs (tests, tooling) have no pinned extension — a regular copy is fine there.
        if torch.cuda.is_available():
            out = alloc_pinned_tensor(*t.shape, dtype=t.dtype)
        else:
            out = torch.empty(*t.shape, dtype=t.dtype)
        out.copy_(t)
        return out

    I, I_loc, lo, hi = _tp_expert_geometry(model_config)
    for name, layers in sources.items():
        out = []
        sliced = False
        for t in layers:
            if name in ("gate_up", "gate_up_scale", "gate_up_global"):
                # row axis = dim 1: gate rows [0, I) + up rows [I, 2I)
                out.append(_pin(torch.cat([t[:, lo:hi], t[:, I + lo : I + hi]], dim=1)))
                sliced = True
            elif name == "down":
                out.append(_pin(t[:, :, lo // 2 : hi // 2]))
                sliced = True
            elif name == "down_scale":
                out.append(_pin(t[:, :, lo // 16 : hi // 16]))
                sliced = True
            else:
                out.append(t)
        sources[name] = out
        if sliced and host_banks and name in host_banks:
            for bank in host_banks[name]:
                bank.dispose()
            host_banks.pop(name, None)
...
def load_expert_banks(
    ...
    * **Slow path** (the original checkpoint): auto-pick **parallel** (the common parallel chunked
    ...
    # The FTW bank path reads the full-width banks per rank: at TP2 the peak is
    # 2x the bank set (+ the ZFS page cache on 2.2, which ignores O_DIRECT) and
    # OOMs for large models. TP2 takes the serial raw path (sliced banks built
    # directly); TP1 keeps the FTW fast path.
    if model_path and is_ftw_checkpoint(model_path) and not dummy:
        ...
        if banks is not None:
            if not banks.tp_sliced:
                _tp_slice_ftw_sources(banks.sources, model_config, banks.host_banks)
