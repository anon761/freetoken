...
def _grouped_decode(
    ...
    *,
    ...
    # must be a multiple of BLOCK_SIZE_KB; the DSV4 TP=1 shapes hit 128, but TP-sliced
    # intermediates (V4.1 at TP2: down packed K = 576) only clear 64 — the kernel is
    # KB-generic (NB = KB//16), so shrink the tile to the largest divisor (JIT
    # specializes per constexpr value; TP=1 shapes keep the fast 128 path).
    BLOCK_SIZE_N = 16
    BLOCK_SIZE_KB = 128
    _NW = 1
    packed_k = packed_cache.shape[2]
    assert packed_k % 32 == 0, (packed_k,)
    while packed_k % BLOCK_SIZE_KB:
        BLOCK_SIZE_KB //= 2
