...
class HostBank:
    ...
    * ``"cuda"`` -- cudaHostAlloc, born pinned+mapped; ``pin()``/``lock()``/``release()`` are no-ops and it never takes LOCKED. See :func:`born_pinned_default`.
    ...
    __slots__ = ("tensor", "addr", "nbytes", "_buf", "_pinned", "_locked", "_disposed")
    ...
    def dispose(self) -> None:
        """Fully retire this bank: unregister pinned pages, drop resident pages, close the mmap.

        The FTW TP>1 path slices full banks into per-rank copies and abandons the originals —
        without dispose the registered pages stay resident and page-locked for the process
        lifetime (~1.5x the serving set). The tensor view becomes INVALID after this; callers
        must have replaced every reference first. No-op after the first call."""
        if getattr(self, "_disposed", False):
            return
        self._disposed = True
        self.tensor = torch.empty(0)  # sever the view before the backing dies
        if not isinstance(self._buf, mmap.mmap):
            return  # born-pinned (cudaHostAlloc): dropping the refs frees via the host allocator
        if self._pinned:
            from freetoken.kernel.pinned import host_unregister

            try:
                host_unregister(self.addr)
            except Exception as exc:  # noqa: BLE001 — never fail the load over a cleanup
                import logging

                logging.getLogger(__name__).warning(
                    f"cudaHostUnregister failed ({exc!r}); pages stay pinned until exit"
                )
            self._pinned = False
        try:
            self._buf.madvise(mmap.MADV_DONTNEED)
        ...
        try:
            ...
            pass
        for i, buf in enumerate(_LIVE_BUFFERS):
            if buf is self._buf:
                del _LIVE_BUFFERS[i]
                break
...
def read_range_into(buf: memoryview | mmap.mmap, path: str, *, file_offset: int, nbytes: int,
    ...
    try:
        ...
        if len(offs) <= 1:
            ...
            # Contiguous segments per worker, NOT a round-robin of chunks: each worker then
            # reads a long sequential run, which is what lets the filesystem's readahead
            # work (~5 GiB/s) instead of ping-ponging offsets (~1 GiB/s). rc: same fix as
            # FTWReader.read_entries.
            nseg = max(1, min(workers, len(offs)))
            per = (len(offs) + nseg - 1) // nseg
            segs = [offs[i : i + per] for i in range(0, len(offs), per)]

            def run_seg(seg: list) -> None:
                for o in seg:
                    rd(o)

            with ThreadPoolExecutor(len(segs)) as ex:
                list(ex.map(run_seg, segs))
    finally:
        os.close(fd)
    return nbytes
