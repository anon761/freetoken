...
def iter_expert_pieces(
    ...
    if kind is QuantKind.FP8_BLOCK:
        # Shared block-fp8 reader (no family hook needed); families with a custom
        # fp8 layout still override via their own iter_expert_pieces hook above.
        from freetoken.models.fp8_block_banks import iter_fp8_block_expert_pieces

        return iter_fp8_block_expert_pieces(
            model_path, config, parallel=parallel, workers=workers, chunk=chunk
        )
    ...
    if kind is QuantKind.W4A16:
        spec_hook = _model_hook(spec, "wna16_expert_spec")
        if spec_hook is None:
            raise NotImplementedError(f"{spec.module} provides no wna16_expert_spec")
        from freetoken.models.wna16_banks import iter_wna16_expert_pieces

        return iter_wna16_expert_pieces(
            model_path, config, spec_hook(model_path, config), parallel=parallel, workers=workers, chunk=chunk
        )
