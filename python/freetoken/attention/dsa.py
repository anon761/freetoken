...
class DSAAttnBackend(DSAIndexerMixin, BaseAttnBackend):
    def __init__(self, config: ModelConfig) -> None:
        ...
        self.config = config
        # TP: the MLA latent KV is shared (MQA-style), only the query/output heads
        # shard -- the backend sees exactly the heads the model feeds it.
        from freetoken.distributed import get_tp_info
        from freetoken.utils import div_even

        self.num_heads = div_even(config.num_qo_heads, get_tp_info().size)
