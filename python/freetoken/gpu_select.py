...
def parse_gpu_memory_ratio(value: str) -> tuple[tuple[str, float], ...]:
    """Parse a per-GPU ratio list (--gpu-memory-ratio, or a --memory-ratio list).

    Two entry forms are accepted:

    - ``<gpu>,<ratio>`` entries separated by ``;`` -- the original form; ``;`` is a
      shell operator, so the whole value must be quoted: ``'0,0.8;1,0.9'``.
    - ``<gpu>:<ratio>`` entries separated by ``,`` or ``;`` -- shell-safe, no quotes:
      ``0:0.8,1:0.9``.

    The GPU is named as in --gpu (an nvidia-smi index or a UUID prefix), the ratio is a
    VRAM fraction in [0, 1]. Raises ValueError on a malformed, duplicate, or out-of-range
    entry. A single ``--memory-ratio`` still applies to every rank without an entry here.
    """
    value = value.strip()
    if ":" in value:
    ...
    for item in (part.strip() for part in items):
        if not item:
            continue
        spec, found, ratio_str = item.partition(sep)
        if not found:
            raise ValueError(f"{item!r}: expected '<gpu>{sep}<ratio>'")
        try:
            gpu = _canonical(spec.strip())
        except ValueError as exc:
            raise ValueError(f"{item!r}: {exc}") from None
        try:
            ratio = float(ratio_str.strip())
        except ValueError:
            raise ValueError(f"{item!r}: ratio must be a number in [0, 1]") from None
        if not 0.0 <= ratio <= 1.0:
            raise ValueError(f"{item!r}: ratio {ratio:g} is outside [0, 1]")
        key = gpu.upper()
        if key in seen:
            raise ValueError(f"{item!r}: GPU {gpu} appears twice")
        seen.add(key)
        entries.append((gpu, ratio))
    if not entries:
        raise ValueError("needs at least one '<gpu>,<ratio>' entry")
    return tuple(entries)


def parse_memory_ratio(value: str) -> "float | tuple[tuple[str, float], ...]":
    """--memory-ratio: a single float (applies to every TP rank) or a per-GPU list
    (same syntax as --gpu-memory-ratio, e.g. ``0:0.8,1:0.9``)."""
    spec = value.strip()
    if not spec:
        raise ValueError("needs a float or a per-GPU list")
    if any(ch in spec for ch in ",;:"):
        return parse_gpu_memory_ratio(spec)
    try:
        return float(spec)
    except ValueError:
        raise ValueError(f"{value!r} is neither a float nor a per-GPU list") from None


def gpu_memory_ratio_arg(value: str) -> tuple[tuple[str, float], ...]:
    """argparse type for --gpu-memory-ratio."""
    try:
        return parse_gpu_memory_ratio(value)
    except ValueError as exc:
        raise argparse.ArgumentTypeError(str(exc)) from exc


def memory_ratio_arg(value: str) -> "float | tuple[tuple[str, float], ...]":
    """argparse type for --memory-ratio (float or per-GPU list)."""
    try:
        return parse_memory_ratio(value)
    except ValueError as exc:
        raise argparse.ArgumentTypeError(str(exc)) from exc


def resolve_memory_ratios(
    ...
    """One memory ratio per TP rank.

    ``entries`` are the --gpu-memory-ratio overrides (GPU spec -> ratio), ``specs`` the
    --gpu entries in rank order and ``assigned`` their resolved full UUIDs when NVML is
    available. Each rank takes the ratio of the entry naming its GPU; a rank with no entry
    keeps ``default`` (--memory-ratio). Raises ValueError when an entry names a GPU that is
    not among the served ranks or when two entries resolve to the same rank.

    An entry is matched against the raw --gpu entries first (the same index/UUID naming the
    user typed); a cross-namespace entry (a UUID against index --gpu entries, or the reverse)
    falls back to resolving it through NVML.
    """
    if assigned is not None:
        n_ranks = len(assigned)
    elif specs:
        n_ranks = len(specs)
    else:
        n_ranks = tp_size

    ratios = [default] * n_ranks
    matched: set[int] = set()
    for spec, ratio in entries:
        rank = _ratio_rank(spec, specs, assigned, n_ranks)
        if rank in matched:
            raise ValueError(f"--gpu-memory-ratio {spec}: another entry already covers this GPU")
        matched.add(rank)
        ratios[rank] = ratio
    return tuple(ratios)


def _ratio_rank(spec: str, specs: Sequence[str], assigned: "Sequence[str] | None", n_ranks: int) -> int:
    """The TP rank ``spec`` names, else ValueError."""
    hits = [i for i, s in enumerate(specs) if _same_gpu(spec, s)]
    if len(hits) == 1:
        return hits[0]
    if assigned is not None:
        # cross-namespace (the entry names the GPU differently from --gpu): resolve via NVML
        try:
            resolved = resolve_gpu_uuids([spec])
        except ValueError as exc:
            raise ValueError(f"--gpu-memory-ratio {spec}: {exc}") from None
        if resolved is not None:
            uuid = resolved[0].upper()
            hits = [i for i, a in enumerate(assigned) if a.upper() == uuid]
            if len(hits) == 1:
                return hits[0]
    elif not specs and is_gpu_index(spec) and int(spec) < n_ranks:
        # no --gpu: rank i runs on CUDA ordinal i
        return int(spec)
    raise ValueError(
        f"--gpu-memory-ratio {spec}: GPU is not among the served ranks; name it as in --gpu"
    )


def _same_gpu(a: str, b: str) -> bool:
    if is_gpu_index(a) and is_gpu_index(b):
        return int(a) == int(b)
    if is_gpu_uuid(a) and is_gpu_uuid(b):
        ua, ub = a.upper(), b.upper()
        return ua.startswith(ub) or ub.startswith(ua)
    return False
