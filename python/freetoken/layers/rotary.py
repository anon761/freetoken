...
def get_rope(
    ...
    """Cached rope factory, keyed on geometry AND target device.

    The device must be part of the cache key: CPU-side model construction (tests, the
    FTW converter) would otherwise hand its CPU cos_sin_cache to a later GPU caller
    with the same geometry (flashinfer rejects it with ``cos_sin_cache must be a CUDA
    tensor``). Meta-device callers go through ``_ROPE_DEVICE`` as before.
    """
    t = torch.tensor([])
...
def _get_rope_cached(
    ...
    with torch.device(device):
        return _get_rope(
            ...
            max_position,
            base,
            dict(rope_scaling) if rope_scaling is not None else None,
            is_neox,
        )
