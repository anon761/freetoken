...
class TritonNvfp4MoEKernel(MoEKernel):
    ...
    def layout(self, cfg: MoEConfig) -> dict[str, BankSpec]:
        # TP: the banks carry this rank's intermediate slice (the piece loader
        # pre-slices to the same geometry) -- the offload path shards instead of
        # computing the full experts per rank and double-counting via all_reduce.
