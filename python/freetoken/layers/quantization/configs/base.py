...
class QuantConfig(ABC):
    ...
    def expert_kind(self) -> str | None:
        """The QuantKind this checkpoint declares for the routed experts, or None when the
        config does not say. Group/algo dialects override; the caller (``detect_expert_quant``)
        falls back to the legacy heuristic when this is None."""
        return None

    def has_kind(self, kind: str) -> bool:
        """Whether any declared group/scheme uses ``kind`` (dialects with groups override)."""
        return False
