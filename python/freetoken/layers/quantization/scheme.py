...
class QuantKind(Enum):
    ...
    NVFP4 = "nvfp4"
    MXFP4 = "mxfp4"
    # Weight-only INT4 (AutoRound / AutoGPTQ "WNA16"): packed int4 qweight +
    # packed qzeros + fp16 group scales.
    W4A16 = "w4a16"
...
def fp8_block_scheme(scale: str, block: tuple[int, int] = (FP8_BLOCK, FP8_BLOCK)) -> QuantScheme:
    return QuantScheme(QuantKind.FP8_BLOCK, WeightDesc("e4m3", block, scale), {"weight", "weight_scale_inv"})
...
def wna16_scheme(group_size: int = WNA16_GROUP) -> QuantScheme:
    """AutoRound / AutoGPTQ WNA16: packed int4 ``qweight`` + packed ``qzeros`` + fp16 group scales."""
    return QuantScheme(
        QuantKind.W4A16,
        WeightDesc("int4", (1, group_size), "fp16"),
        {"weight", "weight_zero", "weight_scale"},
    )
