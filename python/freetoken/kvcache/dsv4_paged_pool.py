...
class DSV4PagedKVCache(BaseKVCachePool):
    ...
    def _alloc_buffers(self) -> None:
        ...
        for L in range(self._n_layers):
            ...
            owns_indexer = sizes.idx_blocks[L] is not None  # K-cache owners only
            if owns_indexer:
                ...
                # Indexer K-write state ring (V4: the ratio-4 indexer compressor;
                # V4.1: the wk/k_norm K path writes completed rows directly — the
                # ring stays for the mixin contract).
    ...
    def alloc_swa(self, full_indices: torch.Tensor) -> None:
        ...
        # A full page must never be handed out while it still holds a live window mapping:
        # a free that returned the full slot without unbinding its window would leak that
        # window page, and the next reuse of the full page would overwrite the mapping
        # (silent swa-slot leak, caught only by check_integrity at idle). Unbind + return
        # any stale window page BEFORE binding fresh ones.
        stale = self.full_to_window[fbases]
        stale = stale[stale >= 0]
        if stale.numel():
            logger.warning(
                f"alloc_swa: returning {int(stale.numel())} stale window page(s) for reused "
                f"full pages {fbases.tolist()}"
            )
            self.full_to_window[(fbases[:, None] + offsets).flatten()] = -1
            self._win_alloc.free(torch.div(stale, P, rounding_mode="floor") * P)
