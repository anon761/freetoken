...
def ring_size_for_ratio(ratio: int) -> int:
    ...
    ``coff * ratio`` with the CSA-overlap coff=2 reserved for ratio 4 (V4-Flash);
    every other ratio keeps coff=1 — ratios 1/2 arrive with V4.1's compressors
    (no overlap transform). Ratio 4 stays 8 and 128 stays 128, exactly as before.
    """
...
def dsv4_cache_per_page(args, swa_ratio: float, P: int = 128) -> int:
    ...
    indexer_layers = getattr(args, "indexer_layer_ids", ())
    indexer_k_layers = getattr(args, "indexer_k_layer_ids", ())
    total = 0
    for L, ratio in enumerate(tuple(args.compress_ratios)[: args.n_layers]):
        ...
        total += (P // ratio) * kv_b
        # Indexer KV: owner indexers only (V4: ratio-4; V4.1: indexer_layer_ids).
        if ratio == 4 or L in indexer_k_layers:
            total += (P // ratio) * idx_b
            # Indexer compress-state ring: its own pool (fp32), sized off
            # the window pages -> swa_ratio-scaled per full page.
            total += round(swa_ratio * ring_size_for_ratio(ratio)) * _idx_state_bytes(args)
...
def dsv4_kv_unit_bytes(args, P: int = 128) -> int:
    ...
    for L, ratio in enumerate(tuple(args.compress_ratios)[: args.n_layers]):
        ...
        if ratio == 4 or L in indexer_k_layers:
            per_page += (P // ratio) * idx_b  # indexer KV
...
def dsv4_window_unit_bytes(args, P: int = 128) -> int:
    ...
    for L, ratio in enumerate(ratios):
        ...
        if ratio == 4 or L in indexer_k_layers:
            per_page += ring_size_for_ratio(ratio) * _idx_state_bytes(args)  # indexer ring
...
def dsv4_pool_sizes(
    ...
    for L, ratio in enumerate(tuple(args.compress_ratios)[: args.n_layers]):
        ...
        owns_indexer_k = ratio == 4 or L in indexer_k_layers
        idx_blocks.append(full_token // ratio if owns_indexer_k else None)
        state_slots.append(n_win_pages * rs)
        ring_sizes.append(rs)
        idx_state_slots.append(n_win_pages * rs if owns_indexer_k else None)
