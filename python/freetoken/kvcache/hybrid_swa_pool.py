...
def _swa_per_req_swa_floor(config) -> int:
    """One request's NON-EVICTABLE swa while it decodes, in tokens:
      ...
      - its own decode tail up to the first out-of-window free: the driver runs only every
        ``swa_eviction_interval`` forwards and floors its frees at the committed length, so the
    ...
    from freetoken.scheduler.cache import _SWA_RETAIN_GAP, _resolve_swa_eviction_interval

    interval = _resolve_swa_eviction_interval(getattr(config, "swa_eviction_interval", None))
    ...
    floor = locked + window + interval + 2 * ps
    if getattr(config, "special_token_ckpt", False):
        floor += window + _SWA_RETAIN_GAP + interval
    return floor
