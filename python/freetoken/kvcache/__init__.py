...
def create_kvcache_pool(
    ...
    if len(kv_specs) == 1 and kv_specs[0].attn_type == _AttnType.QSA:
        ...
        # TP: the GQA kv heads shard across ranks (LinearQKVMerged contract —
        # allow_replicate mirrors its kv-head handling).
        from freetoken.distributed import get_tp_info
        from freetoken.utils import div_even

        return QSAKVCache(
            num_kv_heads=div_even(spec.num_kv_heads, get_tp_info().size, allow_replicate=True),
            # MTP draft layers (mtp_enabled) continue ids past num_layers but store
            # into this pool via the group's layer map — cover them in the bounds check.
            num_layers=max(
                model_config.num_layers,
                (max(spec.layer_ids) + 1) if spec.layer_ids else model_config.num_layers,
            ),
            head_dim=spec.head_dim,
            num_pages=num_pages,
            page_size=page_size,
