...
_MODEL_REGISTRY: dict[str, ModelSpec] = {
    ...
    # DeepSeek-V4.1-Flash (model_type deepseek_v41): 40 mHC(4)-MoE layers, CSA2
    # sparse attention with kv-source sharing (Phase 3; Phase 1 serves the
    # skeleton with generic full attention), Engram n-gram memory (Phase 4),
    # DSpark draft (Phase 6) — the DSV4.1 plan. Same root-less checkpoint
    # naming as V4-Flash plus the top-level ``embed``.
    "DeepseekV41ForCausalLM": ModelSpec(
        "freetoken.models.deepseek_v41",
        "DeepseekV41ForCausalLM",
        # the checkpoint has no ``model.`` root (same naming as V4-Flash)
        checkpoint_roots=(("model.layers", "layers"), ("model.head", "head")),
        packed_modules_mapping=_EXPERTS_W123_PACKED,
        # the head/embed ship bf16, and the compressor's wkv/wgate serve unquantized
        # (V4 precedent — the checkpoint's .scale sidecars are ignored there too);
        # the fp8 config has no modules_to_not_convert
        unquantized_modules=("head", "embed", "*.compressor.wkv", "*.compressor.wgate",
                             "*.indexer.weights_proj", "*.indexer.wk"),
    ),
    ...
    # Text-only sibling (model_type qwen4_exp_text): the text tower's own architectures entry
    # in a language-model-only export. No wrapper, so the weights sit under the bare ``model.``
    # root instead of ``model.language_model.`` -- no checkpoint_roots mapping.
