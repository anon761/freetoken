...
def parse_config(hf_config: Any) -> ModelConfig:
    ...
    # Routed experts and the dense MXFP8 projections come from the shared dialect layer
    # (modelopt MIXED_PRECISION: quantized_layers map). detect_expert_quant resolves the
    # experts from that map; the dense role is an MXFP8 probe. The reader keeps them
    # native; the layers take their scheme from the same QuantConfig.
    from freetoken.models.config import detect_expert_quant
    from freetoken.models.quant import quant_config_of, quant_kind_str

    expert_quant = detect_expert_quant(hf_config)
    try:
        _qc = quant_config_of(hf_config)
    except NotImplementedError:
        _qc = None

    def _kind(prefix: str) -> str:
        try:
            return quant_kind_str(_qc, prefix) if _qc is not None else "none"
        except Exception:  # noqa: BLE001
            return "none"

    dense_mxfp8 = _kind("model.layers.0.mlp.gate_up_proj") == "mxfp8"
