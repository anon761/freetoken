...
class Qwen4ExpAttention(BaseOP):
    ...
    def __init__(self, config: ModelConfig, layer_id: int, *, prefix: str = "") -> None:
        ...
        # TP: GQA head split (24 q / 2 kv -> 12 / 1 per rank at tp=2). The qkv
        # projection is already column-parallel over the merged [2q|kv|kv] rows;
        # o_proj becomes row-parallel with all_reduce.
        ...
        self._qkv_split_local = [
            self.local_q * self.head_dim * 2,
            self.local_kv * self.head_dim,
            self.local_kv * self.head_dim,
        ]
        self.o_proj = (
            LinearRowParallel(
    ...
    def forward(self, x: torch.Tensor, batch: Batch) -> torch.Tensor:
        qg, k, v = self.qkv_proj.forward(x).split(self._qkv_split_local, dim=-1)
        qg = qg.view(-1, self.local_q, self.head_dim * 2)
        q = qg[..., : self.head_dim].contiguous()
        gate = qg[..., self.head_dim :].reshape(-1, self.local_q * self.head_dim)
        k = k.contiguous().view(-1, self.local_kv, self.head_dim)
        v = v.contiguous()
        self.q_norm.forward_inplace(q)
        self.k_norm.forward_inplace(k)
        q, k = self.rotary.forward(
            batch.positions, q.view(-1, self.local_q * self.head_dim), k.view(-1, self.local_kv * self.head_dim)
        )
        index = self.indexer.forward(x)
        o = get_global_ctx().attn_backend.qsa_forward(
            q.view(-1, self.local_q, self.head_dim), k, v, index, self.layer_id, batch
        )
        gated = o.reshape(-1, self.local_q * self.head_dim) * torch.sigmoid(gate)
