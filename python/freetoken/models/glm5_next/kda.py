...
class Glm5NextKDA(BaseOP):
    ...
    def __init__(self, config: ModelConfig, layer_id: int, *, prefix: str = ""):
        ...
        # TP geometry: heads shard across ranks (q|k|v|b are per-head); the low-rank
        # gate bottlenecks f_a|g_a (d-dim, head-unstructured) stay replicated, as do
        # the norms. The loader slices the fused in_proj rows accordingly (weight.py).
        tp = get_tp_info()
        self.local_heads = div_even(h, tp.size)
        self.local_proj_size = self.local_heads * d
        if tp.size == 1:
            ...
            # Fused GEMM with a rank-dependent layout: q|k|v|b carry this rank's
            # head slice, f_a|g_a the full d-dim bottleneck. LinearColParallelMerged
            # can't express the mixed sharding (it divides EVERY part), so allocate
            # the local shape directly and let the loader fill the rows. A quant
            # scheme on the fused in_proj (block-fp8 resident checkpoints) stays
            # TP=1: the block scales shard on a 128-grid the mixed layout breaks.
            if config.quant.scheme_for(f"{prefix}.in_proj") is not None:
                raise NotImplementedError(
                    "glm5_next KDA quantized resident weights support TP=1 only"
                )
            self._in_proj_split_local = [
                3 * self.local_proj_size, self.local_heads, d, d,
            ]
            full_split = [p, p, p, h, d, d]  # full q|k|v|b|f_a|g_a
            self.in_proj = _LinearTPImpl(
                args.hidden_size, sum(full_split),
                args.hidden_size, sum(self._in_proj_split_local),
                has_bias=False,
            )
            # Low-rank gate up-projections (128 -> 8192): forget gate and output
            # gate. Column-parallel (full d input, this rank's H*D channels out).
            self.f_b_proj = LinearColParallelMerged(d, [p], has_bias=False, quant_config=config.quant, prefix=f"{prefix}.f_b_proj")
            self.g_b_proj = LinearColParallelMerged(d, [p], has_bias=False, quant_config=config.quant, prefix=f"{prefix}.g_b_proj")
            # One merged depthwise conv over the q|k|v stream (channel-axis concat);
            # channels follow the rank's head slice.
            self.conv1d = _DepthwiseConv1d(3 * self.local_proj_size, self.conv_kernel_size)
            ...
            self.A_log = torch.empty(self.local_heads, dtype=torch.float32)
            self.dt_bias = torch.empty(self.local_proj_size, dtype=torch.float32)
            self.o_norm = GatedRMSNorm(d, eps=args.norm_eps, activation="sigmoid")
            # o_proj row-parallel: the input is this rank's head slice, outputs
            # all_reduced.
            self.o_proj = LinearRowParallel(p, args.hidden_size, has_bias=False, quant_config=config.quant, prefix=f"{prefix}.o_proj")
    ...
    def forward(self, hidden_states: torch.Tensor) -> torch.Tensor:
        ...
        d = self.head_dim
        h = self.local_heads  # TP-local: q/k/v/b/g and the state slots carry this rank's heads
        p = self.local_proj_size
        conv_dim = 3 * p
