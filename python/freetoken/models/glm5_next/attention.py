...
class Glm5NextAttention(BaseOP):
    def __init__(self, config: ModelConfig, layer_id: int, *, prefix: str = ""):
        ...
        # TP: per-head projections shard on the head axis (q_b/kv_b column-parallel
        # over the head-major rows), o_proj row-parallel with all_reduce. The MLA
        # latent (kv_a) and the lora bottleneck (q_a) are shared across heads --
        # replicated. The indexer is replicated by design (full head set, its
        # per-token selection is rank-invariant).
        self._tp_size = get_tp_info().size
        self.local_heads = div_even(self.num_heads, self._tp_size)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        h = self.local_heads  # TP-local head count; q/kv_b carry only this rank's heads
        ...
        q_nope = q.view(t, h, self.qk_head_dim)
        ...
        return self.o_proj.forward(o.reshape(t, h * self.v_head_dim))
