        ...
        for idx, part in enumerate(parts):
            if base.endswith(part):
                ...
                if len(slots) < len(parts):
                    return []
                del buf[key]
                return [(key + ".weight", cat_fused_rows(key, [slots[i] for i in range(len(parts))]))]
    return None


def ct_fp8_fuse(base: str, parts_tuple: tuple, buf: dict, groups: dict[str, tuple[str, ...]]):
    """Buffer a compressed-tensors FP8 fusion part ``(weight fp8, per-row scale fp32)``;
    emit the concatenated ``.weight`` + ``.weight_scale`` once complete, ``[]`` while
    incomplete, ``None`` if ``base`` is not a fusion part of any group in ``groups``.

    The model's FP8 linears are per-output-row scaled, so weight and scale concatenate on
    the output axis exactly like the modelopt path. Unlike modelopt, compressed-tensors
    FP8 has no static input scale (dynamic activations), so none is emitted."""
    for fused_suffix, parts in groups.items():
        for idx, part in enumerate(parts):
            if base.endswith(part):
                key = base[: -len(part)] + fused_suffix
...
_FLOAT8_DTYPES = ()
try:  # float8 dtypes exist on every torch this codebase targets; guard anyway
    _FLOAT8_DTYPES = (torch.float8_e4m3fn, torch.float8_e5m2)
except AttributeError:  # pragma: no cover
    pass


def cat_fused_rows(key: str, rows: list) -> torch.Tensor:
    """torch.cat for a fusion group with a diagnostic for Float8 members.

    torch refuses ANY cat involving Float8 storage ("Promotion for Float8 Types is
    not supported"). The usual cause: the checkpoint quantized the fused projections
    themselves (attention q/k/v/o, shared expert, lm_head) — a layout FreeToken
    cannot serve (the raw loader would fail identically; the model's fused buffers
    have no fp8 variant). Raise an actionable message instead of the cryptic
    promotion error."""
    if _FLOAT8_DTYPES and any(t.dtype in _FLOAT8_DTYPES for t in rows):
        raise ValueError(
            f"Checkpoint not FreeToken-compatible: fusion group {key!r} contains Float8 "
            "(fp8-quantized dense projections, e.g. attention/shared-expert/lm_head). "
            "FreeToken cannot load this quantization layout — the model stays vLLM-only."
        )
