...
class Qwen3_5Attention(BaseOP):
    ...
    TP: GQA head split (q evenly, kv replicated when fewer kv heads than ranks);
    ``qkv_proj`` is already column-parallel over the merged [2q|kv|kv] rows, ``o_proj``
    becomes row-parallel with all_reduce.
    """
    ...
    def __init__(self, config: ModelConfig, layer_id: int, *, prefix: str = ""):
        ...
        self.local_q = div_even(self.num_q, tp.size)
        self.local_kv = div_even(self.num_kv, tp.size, allow_replicate=True)
        self.head_dim = head_dim
        self.qo_attn_dim = self.num_q * head_dim
        self.kv_attn_dim = self.num_kv * head_dim
        self.local_qo_attn_dim = self.local_q * head_dim
        self.local_kv_attn_dim = self.local_kv * head_dim
        ...
        self._qkv_split_local = [
            self.local_qo_attn_dim * 2, self.local_kv_attn_dim, self.local_kv_attn_dim,
        ]
    ...
    def _project(self, x: torch.Tensor):
        """Returns (q, k, v, gate): q [N, local_q, head_dim] post qk-norm+rope,
        k [N, local_kv*head_dim] post norm+rope, v [N, local_kv*head_dim],
        gate [N, local_q*head_dim]."""
        positions = get_global_ctx().batch.positions
        qkv = self.qkv_proj.forward(x)
        qg, k, v = torch.split(qkv, self._qkv_split_local, dim=-1)
        qg = qg.view(-1, self.local_q, self.head_dim * 2)
        q = qg[..., : self.head_dim].contiguous()  # [N, local_q, head_dim]
        gate = qg[..., self.head_dim :].reshape(-1, self.local_qo_attn_dim)
        k = k.view(-1, self.local_kv, self.head_dim).contiguous()
        v = v.contiguous()  # split view has the qkv row stride; the KV store needs contiguous
        q = self.q_norm.forward(q).reshape(-1, self.local_qo_attn_dim)
        k = self.k_norm.forward(k).reshape(-1, self.local_kv_attn_dim)
        q, k = self.rotary.forward(positions, q, k)
        return q.view(-1, self.local_q, self.head_dim), k, v, gate

    def _combine(self, attn_out: torch.Tensor, gate: torch.Tensor) -> torch.Tensor:
        gated = attn_out.reshape(-1, self.local_qo_attn_dim) * torch.sigmoid(gate)
