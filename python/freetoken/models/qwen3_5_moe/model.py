...
class Qwen3_5Model(BaseOP):
    def __init__(self, config: ModelConfig, *, prefix: str = "model"):
        ...
        # MTP draft head: stash the pre-final-norm residual stream for the draft input.
        self._mtp_streams = config.mtp_enabled
    ...
    def forward(self, input_ids: torch.Tensor) -> torch.Tensor:
        ...
        x, hidden = self.norm.forward_add_residual(x, residual)
        if self._mtp_streams:
            # ``hidden`` is the summed pre-norm residual stream (the MTP draft's R_last).
            get_global_ctx().batch.mtp_streams = hidden
        return x
