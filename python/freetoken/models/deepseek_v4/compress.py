...
class Compressor(BaseOP):
    ...
    def _score(self, x: torch.Tensor) -> torch.Tensor:
        """Gate logits for the gated pooling (V4: the learned wgate projection)."""
        return self.wgate.forward(x)

    def _score_decode(self, x: torch.Tensor) -> torch.Tensor:
        return bf16_linear_fp32(x, self.wgate.weight)
    ...
    def decode_step(
        ...
        score = self._score_decode(x).view(B, 1, item)
        idx_mod = pos % ratio  # [B]
