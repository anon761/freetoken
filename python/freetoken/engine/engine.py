    ...
    *,
    ...
    for key, param in model_state.items():
        if param.dtype in fp8_dtypes:
            ...
            # UE8M0 exponent codes (V4.1's MXFP8 scales): 127 encodes scale 1.0 —
            # same reasoning as the uint8 weight_scale_inv branch below. randn is
            # not implemented for the dtype and 0 would collapse every scale to
            # 2^-127 (i.e. zero the model).
...
class ForwardOutput(NamedTuple):
    ...
    # Pre-lm_head residual streams of the last position per request ([bs, hc*hidden];
    # MTP models, eager decode only -- None under CUDA-graph replay or without MTP).
...
class Engine:
    def __init__(self, config: EngineConfig):
        ...
        # Startup KV sizing keeps the cross-rank MIN: TP pools are allocated with
        # equal sizes on every rank, so the budget must fit the LEAST-free GPU.
        # Sizing from MAX (upstream) only works on dedicated cards — with a
        # side-loaded rank (e.g. a TTS worker sharing GPU 0) the pool would OOM
        # that rank at allocation time.
        init_free_memory = free_min
        ...
        # allocator figure attributes it — if they diverge, the extra is OUTSIDE the torch
        # allocator (driver-charged, e.g. pinned-mapped memory) and the snapshot pinpoints it.
        logger.info_rank0(
            f"Weights resident: {mem_GB(self._weights_bytes)} "
            f"(torch allocator {torch.cuda.memory_allocated() / 2**30:.2f} GiB / "
        ...
        self.dspark_offload_cache = None
        ...
        new_free = self._sync_get_memory()[0]
        ...
        # Side-loaded processes (TTS/upscale workers, desktop apps) can grab VRAM between
        # the baseline measurement and this alloc — a 30-min expert-bank build is plenty of
        # window. Re-measure and clamp so the pool never chases memory that is already gone.
        current_free = self._sync_get_memory()[0]
        if current_free < new_free:
            logger.warning_rank0(
                f"free VRAM dropped {mem_GB(new_free - current_free)} since the post-load "
                "measurement (side-loaded process?); clamping the KV budget"
            )
            available_memory -= new_free - current_free
        ...
        # MTP verify CUDA graph (Phase 3), opt-in: a fixed prefill extend, captured with the
        # QSA backend's static verify buffers -- one graph per padded batch size (mirroring
        # the decode graph list) so a small round does not pay the largest shape.
        self.verify_graph_runner = None
        _decode_bs = list(getattr(self.graph_runner, "graph_bs_list", []))
        if (
            os.environ.get("FREETOKEN_MTP_VERIFY_GRAPH") == "1"
            and _decode_bs
            and hasattr(self.attn_backend, "init_capture_verify")
        ):
            from freetoken.engine.verify_graph import VerifyGraphRunner
            from freetoken.env import ENV

            _t = int(ENV.MTP_DRAFT_TOKENS.value) + 1
            _max_req = max(1, config.max_running_req)
            _bs_list = sorted({b for b in _decode_bs if b <= _max_req} | {_max_req})
            self.verify_graph_runner = VerifyGraphRunner(
                ...
                device=self.device,
                padding_slot=(
                    self.linear_state_pool.padding_slot
                    if self.linear_state_pool is not None
                    else 0
                ),
            )
        ...
        if (
            os.environ.get("FREETOKEN_MTP_DRAFT_GRAPH") == "1"
            and getattr(self.model, "mtp", None) is not None
            and _decode_bs
            and hasattr(self.attn_backend, "init_capture_draft")
        ):
            from freetoken.engine.draft_graph import DraftGraphRunner

            _q4 = getattr(config.model_config, "qwen4_args", None)
            _width = getattr(_q4, "ple_state_width", None) or config.model_config.hidden_size
            _max_req = max(1, config.max_running_req)
            _dbs_list = sorted({b for b in _decode_bs if b <= _max_req} | {_max_req})
            self.draft_graph_runner = DraftGraphRunner(
                graph_runner=self.graph_runner,
                model=self.model,
                attn_backend=self.attn_backend,
                bs_list=_dbs_list,
    ...
    def _init_offload_moe_cache(self, config: EngineConfig) -> OffloadMoeCache:
        ...
        verify_cpu = bool(getattr(config, "moe_verify_cpu", False) or ENV.MOE_VERIFY_CPU)
        ...
        try:
            banks = load_expert_banks(
                ...
                workers=config.expert_load_workers,
        ...
        if config.moe_cache_auto:
            ...
            from freetoken.engine.cache_budget import expert_bytes_per_slot as _eps

            logger.info_rank0(
                f"Bank ledger: per_slot={_eps(banks.sources) / 2**20:.2f} MiB, "
                f"host-pinned banks "
                f"{sum(getattr(b, 'nbytes', 0) for bss in (banks.host_banks or {}).values() for b in bss) / 2**30:.2f} GiB, "
                f"free now {mem_GB(self._sync_get_memory()[0])}"
            )
        ...
        if cache.decode_target in ("cpu", "hybrid") or verify_cpu:
            self._init_cpu_moe_executor(config, cache, layers)
        self.ctx.moe_offload_cache = cache
        self.moe_offload_cache = cache
        logger.info_rank0(
            f"MoE: strategy={config.moe_strategy} decode_target={cache.decode_target} "
            f"verify_cpu={cache.verify_cpu} cache_size={cache.cache_size} "
            f"cpu_layers={len(cpu_layer_ids)} quant={banks.quant_format}"
        )
        self._init_dspark_offload_cache(config)
        return cache

    def _init_dspark_offload_cache(self, config: EngineConfig):
        """Load the DSpark draft's dedicated expert cache (E=128 vs the target's 384).

        The draft is marked ``offload_excluded``, so the target's method pick and
        ``attach_offload_moe_cache`` above never touched its layers; it owns a separate
        method/cache keyed by draft stage, with banks under their own FTW kind.
        """
        getter = getattr(self.model, "dspark_draft", None)
        draft = getter() if callable(getter) else None
        if draft is None:
            return None
        if config.model_config.decode_target != "gpu":
            # The draft would need its own CPU/hybrid executor; DSpark spec decode is a
            # GPU-expert optimization, so reject loud rather than serve a half-wired cache.
            raise NotImplementedError(
                "DSpark spec decoding supports GPU expert decode only "
                f"(model decode_target={config.model_config.decode_target!r})"
            )
        args = config.model_config.dsv4_args
        num_stages = int(args.num_nextn_predict_layers)
        num_experts = int(args.dspark_n_routed_experts)
        method = draft.layers.op_list[0].ffn.experts.quant_method
        if method is None:
            raise RuntimeError("DSpark draft is enabled but its MoE layers carry no expert method")
        from freetoken.models.weight import load_dspark_banks

        banks = load_dspark_banks(
            config.model_path, config.model_config, method=method, num_stages=num_stages,
            device=self.device, dtype=self.dtype, dummy=config.use_dummy_weight,
        )
        # The minimum viable slot cache (one full draft stage); stage 3 trades VRAM for
        # decode hit rate once acceptance is measurable.
        cache = draft.build_offload_cache(
            banks, cache_size=num_experts, device=self.device, decode_target="gpu"
        )
        self.dspark_offload_cache = cache
        self.ctx.dspark_offload_cache = cache
        logger.info_rank0(
            f"DSpark draft cache: {num_stages} stages x {num_experts} experts, "
            f"{cache.cache_size} GPU slots, {banks.quant_format} banks"
        )
        return cache
    ...
    def _init_cpu_moe_executor(self, config: EngineConfig, cache, layers) -> None:
        ...
        # round a batch up to the largest captured size; cover both. --moe-verify-cpu
        # additionally packs max_running_req * (k+1) tokens into one verify call.
        from freetoken.env import ENV

        max_tokens = max(
            config.max_running_req,
            config.cuda_graph_max_bs or 0,
            config.max_running_req * (int(ENV.MTP_DRAFT_TOKENS.value) + 1),
            1,
        )
    ...
    def _sync_get_memory(self) -> Tuple[int, int]:
        ...
        if imbalance > 12 * 1024 * 1024 * 1024:
            # Egregious — this is rank mis-pinning (e.g. both ranks on one card),
            # not benign side-load. KV sizing uses the cross-rank MIN, so a moderate
            # imbalance is safe (the shared rank just gets less cache); a full-card
            # gap means something is wired wrong.
            logger.error(
                f"Memory across TP ranks are imbalanced:"
                f" min {mem_GB(min_free_memory)}, max {mem_GB(max_free_memory)}"
            )
            raise RuntimeError("Memory across TP ranks are imbalanced")
        if imbalance > 2 * 1024 * 1024 * 1024:
            logger.warning(
                f"Memory across TP ranks differs by {mem_GB(imbalance)}"
                f" (min {mem_GB(min_free_memory)}, max {mem_GB(max_free_memory)})"
                f" — KV/cache pools are sized for the least-free rank"
            )
    ...
    def forward_batch(self, batch: Batch, args: BatchSamplingArgs) -> ForwardOutput:
        ...
        streams = (
            self.graph_runner.mtp_streams(batch)
            if use_graph
            else getattr(batch, "mtp_streams", None)
        )
        return ForwardOutput(next_tokens_gpu, next_tokens_cpu, copy_done_event, streams)

    def extend_forward(self, batch: Batch) -> tuple[torch.Tensor, "torch.Tensor | None"]:
        """Eager full-model forward over a prefill-phase batch WITHOUT per-request
        last-row selection, sampling, or complete_one bookkeeping -- the MTP verify /
        re-extend path. Returns (full [T, vocab] logits, residual streams [T, hc*hidden])."""
        assert torch.cuda.current_stream() == self.stream
        with self.ctx.forward_batch(batch), self.model.forward_host_ctx(batch, False):
            logits = self.model.forward_logits_all()
        if self.cpu_moe_executor is not None:
            self.cpu_moe_executor.raise_if_unhealthy()
        return logits, getattr(batch, "mtp_streams", None)
