...
def split_count(b: int, m: int, h: int, topk: int, device, allow_multi_query: bool = False) -> int:
    ...
    ``allow_multi_query`` selects the decode split for a multi-query batch (the DSpark decode
    verify): the split geometry depends only on b/h/topk, so an m>1 batch gets the exact split a
    single-query decode of the same shape would."""
    if m != 1 and not allow_multi_query:
        return 0
...
def sparse_attn_paged(
    ...
    n_splits = split_count(b, m, h, topk, q.device, allow_multi_query=allow_multi_query_split)
    if n_splits:
