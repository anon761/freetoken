...
const auto kNCCLDtypeMap =
    std::unordered_map<DLDataType, ncclDataType_t, DLDataTypeHash>{
        ...
        // uint8: byte-transparent transport for the fp8 quantized-wire
        // all_gather (FREETOKEN_TP_REDUCE_FP8) — NCCL's own fp8 collectives are
        // sm90-gated/broken on sm86, so the raw bytes are moved instead.
        // all_reduce() rejects uint8 (an integer sum wraps silently).
        {{DLDataTypeCode::kDLUInt, 8, 1}, ncclUint8},
        // e4m3: a direct fp8 transport for all_gather. all_reduce() rejects it
        // (NCCL needs sm90 for fp8 reductions); the flag's reduce path uses a
        // uint8 all_gather plus a local fp32 sum, not this entry.
        {{DLDataTypeCode::kDLFloat8_e4m3fn, 8, 1}, ncclFloat8e4m3},
    };
...
public:
  ...
  auto all_reduce(tvm::ffi::TensorView t, std::string op) const -> void {
    ...
    // The dtype map is shared with all_gather, so guard the reduce-only
    // invariants here: a uint8 sum wraps silently, and NCCL rejects fp8
    // reductions before sm90 (the fp8 wire format gathers uint8 and sums
    // locally instead).
    RuntimeCheck(!(t.dtype().code == DLDataTypeCode::kDLUInt && t.dtype().bits == 8),
                 "uint8 all_reduce is not supported (an integer sum would wrap "
                 "silently); the fp8 wire format uses all_gather + a local sum");
    RuntimeCheck(t.dtype().code != DLDataTypeCode::kDLFloat8_e4m3fn,
                 "fp8 all_reduce is rejected by NCCL before sm90; the fp8 wire "
                 "format uses a uint8 all_gather + a local fp32 sum");
