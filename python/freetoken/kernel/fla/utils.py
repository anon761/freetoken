...
def tensor_cache(fn: Callable[..., torch.Tensor]) -> Callable[..., torch.Tensor]:
    ...
    Entries pinned with ``wrapper.pin_all`` (or the module-level ``pin_all_tensor_caches``)
    are never evicted: a captured CUDA graph may reference a cached result tensor, and
    eviction would free it under the graph -> an illegal memory access on replay.
    """
    ...
    def wrapper(*args: Any, **kwargs: Any) -> Any:
        ...
        for last_args, last_kwargs, last_result in pinned_entries:
    ...
    def pin_all() -> None:
        pinned_entries.extend(cache_entries)
        cache_entries.clear()

    wrapper.pin_all = pin_all
    _CACHES.append(wrapper)
    return wrapper


def pin_all_tensor_caches() -> None:
    """Pin the current results of every ``@tensor_cache`` function.

    Called once after capturing a CUDA graph whose kernels reference the cached tensors
    (the MTP verify graph); the entries then survive later eviction-driven reuse.
    """
    for wrapper in _CACHES:
        wrapper.pin_all()
