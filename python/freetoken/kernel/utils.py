...
# libnccl sonames in preference order: the unversioned dev symlink (what `-lnccl`
# wants) then the runtime soname a pip/torch wheel ships.
_NCCL_SONAMES = ("libnccl.so", "libnccl.so.2")


def _nccl_link_flags_from(dirs: List[str]) -> List[str]:
    """NCCL link flags for the first directory that actually carries libnccl.

    ``-lnccl`` needs ``libnccl.so``, which the CUDA toolkit does NOT ship and the
    ``nvidia-nccl-cuNN`` wheels do not create either (they only install
    ``libnccl.so.2``). So link the exact soname when that is all we have, and
    bake an rpath in so the built module loads NCCL without ``LD_LIBRARY_PATH``."""
    for d in dirs:
        if not d or not os.path.isdir(d):
            continue
        for soname in _NCCL_SONAMES:
            if os.path.exists(os.path.join(d, soname)):
                flag = "-lnccl" if soname == "libnccl.so" else f"-l:{soname}"
                return [f"-L{d}", flag, f"-Wl,-rpath,{d}"]
    # No copy found: fall back to the plain name and let the linker's default search
    # path resolve it (a system NCCL install), otherwise it errors with the usual
    # actionable message.
    return ["-lnccl"]


def _nvidia_nccl_pip_dir() -> str | None:
    """lib dir of the pip ``nvidia-nccl-cuNN`` package, if installed."""
    try:
    ...
    for dist in distributions():
        ...
        if name.startswith("nvidia-nccl-"):
            located = dist.locate_file("nvidia/nccl/lib")
            if located and os.path.isdir(str(located)):
                return str(located)
    return None


def nccl_link_flags() -> List[str]:
    """Link flags that resolve NCCL for the pynccl AOT build.

    Search order prefers torch's own bundled NCCL (the same library
    ``torch.distributed`` loads, so TP does not end up with two NCCL copies), then
    the pip ``nvidia-nccl-cuNN`` package, then ``NCCL_HOME``/``CUDA_HOME`` and the
    usual system lib dirs."""
    dirs: list[str] = []
    try:
        import torch

        dirs.append(os.path.join(os.path.dirname(torch.__file__), "lib"))
    except Exception:  # noqa: BLE001 - torch always present at runtime; be defensive on import-only
        pass
    pip_dir = _nvidia_nccl_pip_dir()
    if pip_dir:
        dirs.append(pip_dir)
    for env in ("NCCL_HOME", "CUDA_HOME", "CUDA_PATH"):
        root = os.environ.get(env)
        if not root:
            continue
        dirs += [os.path.join(root, "lib"), os.path.join(root, "lib64")]
    dirs += ["/usr/lib/x86_64-linux-gnu", "/usr/local/lib", "/usr/lib64", "/usr/lib"]
    return _nccl_link_flags_from(dirs)
...
def _ensure_cuda_arch_list() -> None:
    """Give tvm-ffi a JIT target when it cannot auto-detect the SM version.

    tvm-ffi's ``_get_cuda_target`` needs NVML/the nvidia driver; container/headless
    setups and a driver/library mismatch make it raise "Could not detect CUDA
    compute_cap". torch already queried the same GPU through the CUDA runtime, so
    derive the arch list from it (only when the env is unset)."""
    if os.getenv("TVM_FFI_CUDA_ARCH_LIST"):
        return
    try:
        import torch
        ...
        if caps:
            os.environ["TVM_FFI_CUDA_ARCH_LIST"] = " ".join(caps)
    except Exception:  # noqa: BLE001 -- best-effort; tvm-ffi then reports its own error
        return
