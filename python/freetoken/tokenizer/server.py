...
def _tokenize_requests(
...
) -> tuple[List[TokenizeMsg], List[torch.Tensor], List[Any], List[UserReply]]:
    ...
    A request carrying images also gets its ``<|image_pad|>`` placeholder expanded to the
    vision tower's soft-token count, plus the raw processor outputs (see tokenizer.vision).
    """
    ...
    for msg in messages:
        try:
            ...
            if vision_processor is not None:
                from .vision import expand_image_tokens, preprocess_messages

                vision = preprocess_messages(vision_processor, msg.text)
                if vision is not None and vision.image_token_id is not None:
                    tokens = expand_image_tokens(tokens, vision.tokens_per_image, vision.image_token_id)
        ...
        ok_vision.append(vision)
    return ok_msgs, ok_tensors, ok_vision, errors
...
def tokenize_worker(
    *,
    ...
    from .vision import load_online_vision

    vision_processor = load_online_vision(tokenizer_path)
    ...
    try:
        while True:
            ...
            if len(detokenize_msg) > 0:
                ...
                sampled_replies = [
                    UserReply(
                        ...
                        mtp_enabled=msg.mtp_enabled,
                        mtp_drafted=msg.mtp_drafted,
                        mtp_accepted=msg.mtp_accepted,
                    )
            ...
            if len(tokenize_msg) > 0:
                ...
                ok_msgs, ok_tensors, ok_vision, errors = _tokenize_requests(
                    tokenize_manager, tokenize_msg, logger, vision_processor
                )
                if errors:
                    send_frontend.put(
                ...
                if ok_msgs:
                    backend = [
                        UserMsg(
                            ...
                            pixel_values=(v.pixel_values if v is not None else None),
                            image_grid_thw=(v.image_grid_thw if v is not None else None),
                        )
                        for msg, t, v in zip(ok_msgs, ok_tensors, ok_vision, strict=True)
                    ]
