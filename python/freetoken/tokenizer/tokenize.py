...
def _load_dsv4_encoder_if_needed(tokenizer: PreTrainedTokenizerBase) -> ModuleType | None:
    """Load the checkpoint's official DSV4 chat encoder when it ships one.

    DeepSeek-V4/V4.1 checkpoints carry ``encoding/encoding_dsv4.py`` (V4) or
    ``encoding/encoding.py`` (V4.1, self-contained) — the authoritative prompt
    format with DSML tool rendering, thinking modes and numeric effort. The
    ``chat_template`` in their tokenizer_config.json is a REDUCED jinja stub
    (no tools), so the template must NOT win when the encoder exists: otherwise
    tool definitions never reach the model (opencode gets degenerate loops
    instead of tool calls). The stub only serves non-DSV4 loaders."""
    model_path = getattr(tokenizer, "name_or_path", None) or getattr(tokenizer, "_name_or_path", "")
    if not model_path:
        return None
    for name in ("encoding_dsv4.py", "encoding.py"):
        encoder_path = os.path.join(str(model_path), "encoding", name)
