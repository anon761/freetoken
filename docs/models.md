...
- The Qwen3.8-Flash-Next NVFP4 exports differ in the draft-head (MTP) packaging:
  RadixArk stacks the experts (bf16 `experts.gate_up_proj`), NVIDIA ships them
  per-expert as block-fp8 (`experts.<e>.*_proj.weight` + `weight_scale_inv`). The
  loader normalizes both to the resident layout, so either checkpoint converts and
  serves unchanged.
